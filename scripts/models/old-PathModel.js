function PathModel(database) {
	...
	that.getDetails = function(info) {
		...
		var details = info;
		return Promise.all([getChildren(info), fillHeiarchy([JSON.parse(JSON.stringify(
			info))])]).then(
			function(data) {
		...
		switch (c.type) {
			...
			case 'node':
				if (c.parentId == 0) {
					...
					return database.node.get(c.languageId, c.bookId, c.parentId).then(
						getNodeInfo);
				}
			default:
	...
	function getFolderAndBookChildren(languageId, parentId) {
		return Promise.all(
			[database.folder.getChildren(languageId, parentId),
				database.book.getChildren(languageId, parentId)
			]).then(
			function(children) {
				var folders, books, i;
				folders = children[0];
	...
	function getNodeChildren(languageId, bookId, parentId) {
		return database.node.getChildren(languageId, bookId, parentId).then(
			function(nodes) {
